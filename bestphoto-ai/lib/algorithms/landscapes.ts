...
export async function selectBestLandscapePhoto(
  files: UploadedFile[]
): Promise<{
  ...
  try {
    ...
    console.log(`Sending ${base64Files.length} files to Gemini Landscapes API`);

    // Send to Gemini Landscapes API
    const response = await fetch("/api/gemini-landscapes", {
      method: "POST",
      headers: { 
    ...
    if (!data.bestPhotoId && typeof data.bestPhotoIndex !== "number") {
      toast.error("AI could not determine the best landscape photo.");
      return { bestPhotoId: null, sortedPhotos: [], reason: "AI could not determine the best landscape photo." };
    }
    ...
    if (!bestPhoto) {
      console.error("Best landscape photo not found:", {
        aiResponse: data.bestPhotoId,
        bestPhotoIndex: data.bestPhotoIndex,
        availableFiles: completedFiles.map(f => f.file.name)
      });
      toast.error("AI selected landscape photo not found in uploaded files.");
      return { bestPhotoId: null, sortedPhotos: [], reason: "AI selected landscape photo not found in uploaded files." };
    }

    console.log(`Best landscape photo found: ${bestPhoto.file.name} (ID: ${bestPhoto.id})`);

    return {
      bestPhotoId: bestPhoto.id,
      sortedPhotos: completedFiles,
      reason: data.reason || "Best landscape photo selected by AI.",
    };
    ...
    if (error instanceof TypeError && error.message.includes('fetch')) {
      ...
      toast.error("An error occurred during landscape photo analysis.");
      return { bestPhotoId: null, sortedPhotos: [], reason: "An error occurred during landscape photo analysis." };
    }
  }
}
